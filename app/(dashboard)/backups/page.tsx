...
export default async function BackupsPage() {
  const items: any[] = await (prisma as any).backup.findMany({
          ...
          <table className="w-full">
            <thead className="bg-muted/20 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">File Path</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Size</th>
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-muted-foreground font-mono max-w-xs truncate">{item.filePath}</td>
                    <td className="px-6 py-4 text-sm text-foreground">{formatBytes(item.fileSize)}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${item.status === "completed" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300"}`}>
                        {item.status}
                      </span>
                    </td>
