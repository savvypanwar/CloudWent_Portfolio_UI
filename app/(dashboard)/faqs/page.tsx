...
export default async function FaqsPage() {
  const items: any[] = await (prisma as any).faq.findMany({
    orderBy: { order: "asc" },
  });
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage frequently asked questions shown on the services page.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-muted/20 border-b border-border">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Question</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Answer</th>
                  ...
                  <tr key={item.id} className="hover:bg-muted/10 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center text-primary-foreground">
                          <HelpCircle className="w-4 h-4" />
                        </div>
                        <div>
                          <p className="font-medium text-foreground text-sm max-w-xs truncate">{item.question}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-muted-foreground max-w-xs truncate">{item.answer}</td>
