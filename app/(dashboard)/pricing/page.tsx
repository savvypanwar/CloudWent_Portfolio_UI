...
export default async function PricingPage() {
  const items: any[] = await (prisma as any).pricingPlan.findMany({
    orderBy: { order: "asc" },
  });
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage pricing plans and packages shown on the services page.
          </p>
        </div>
        <Button asChild variant="primary" size="sm" className="glass-effect shadow-md">
          <Link href="/pricing/new">
            <Plus className="w-4 h-4 mr-2" /> Add Pricing Plan
          </Link>
        </Button>
      </div>
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/10 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${item.featured ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300" : "bg-surface text-foreground dark:bg-gray-900/30 dark:text-gray-300"}`}>
