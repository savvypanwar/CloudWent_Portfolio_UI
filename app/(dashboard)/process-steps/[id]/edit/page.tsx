...
export default function EditProcessStepPage() {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        ...
        Toast.error("Failed to load process step");
        router.back();
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const toastId = Toast.loading("Updating process step...");
  ...
  return (
    <DashboardFormShell backHref="/process-steps" backLabel="Back to Process Steps" title="Edit Process Step" loading={isLoading}>
      <form onSubmit={handleSubmit} className="glass-effect border-border rounded-3xl p-8 md:p-10 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <Input name="step" value={formData.step} onChange={handleChange} required className={dashboardInputClass} />
          </div>
          <div>
        ...
        <div className="flex justify-end gap-4 pt-4 border-t border-border">
          ...
          <Button type="submit" variant="primary" disabled={isSubmitting}>{isSubmitting ? "Saving..." : "Update Process Step"}</Button>
        </div>
      </form>
    </DashboardFormShell>
  );
