...
async function deleteProcessStepAction(formData: FormData) {
  "use server";
  const id = formData.get("id") as string;
  await (prisma as any).processStep.delete({ where: { id } });
  redirect("/process-steps");
}

export default async function ProcessStepsPage() {
  const items: any[] = await (prisma as any).processStep.findMany({
    orderBy: { order: "asc" },
  });
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage how-we-work workflow steps shown on the services page.
          </p>
        </div>
        <Button asChild variant="primary" size="sm" className="glass-effect shadow-md">
          <Link href="/process-steps/new">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-muted/20 border-b border-border">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Step</th>
                  ...
                  <tr key={item.id} className="hover:bg-muted/10 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-primary-foreground">
                    ...
                    <td className="px-6 py-4 text-sm text-foreground font-semibold">{item.title}</td>
                    <td className="px-6 py-4 text-sm text-muted-foreground max-w-xs truncate">{item.description}</td>
                    <td className="px-6 py-4 text-sm text-foreground">{item.page}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right flex items-center justify-end gap-2">
                      <Button variant="outline" size="icon" className="h-8 w-8 border-border hover:bg-muted/50" asChild>
                        <Link href={`/process-steps/${item.id}/edit`}>
