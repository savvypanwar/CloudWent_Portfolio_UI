...
export const AdminDashboard = async () => {
  const [userCount, contactCount, projectCount, appCount, teamMemberCount] = await Promise.all([
    prisma.user.count(),
  ...
  const stats = [
    ...
    { label: "Team Members", value: String(teamMemberCount), note: "team profiles", icon: <Users className="h-5 w-5" />, color: "bg-amber-500" },
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <section className="mb-6 sm:mb-8 rounded-2xl bg-background border border-border p-4 sm:p-6">
        <div className="flex flex-col gap-3 sm:gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="rounded-xl border border-border px-3 sm:px-4 py-2 sm:py-3 w-full sm:w-auto">
            ...
            <p className="text-xs sm:text-sm font-semibold text-gray-950 dark:text-primary-foreground">Admin</p>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
        <div className="xl:col-span-2 rounded-2xl bg-background border border-border p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 mb-4 sm:mb-5">
            <div>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">Manage users, settings, and more.</p>
            </div>
            <BarChart3 className="h-5 w-5 text-muted-foreground flex-shrink-0" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            <Link href="/users" className="rounded-xl border border-border p-3 sm:p-4 hover:border-blue-400 hover:shadow-sm transition-all">
              ...
              <h3 className="text-xs sm:text-sm font-semibold text-gray-950 dark:text-primary-foreground">Manage Users</h3>
            ...
            <Link href="/applications" className="rounded-xl border border-border p-3 sm:p-4 hover:border-blue-400 hover:shadow-sm transition-all">
              ...
              <h3 className="text-xs sm:text-sm font-semibold text-gray-950 dark:text-primary-foreground">Review Applications</h3>
              <p className="mt-1 text-xs text-muted-foreground">See all job applications</p>
            </Link>
            <Link href="/services" className="rounded-xl border border-border p-3 sm:p-4 hover:border-blue-400 hover:shadow-sm transition-all">
              <div className="mb-3 sm:mb-4 inline-flex rounded-lg bg-surface p-2 sm:p-3 text-primary">
                <Settings className="h-4 sm:h-5 w-4 sm:w-5" />
              </div>
              <h3 className="text-xs sm:text-sm font-semibold text-gray-950 dark:text-primary-foreground">Website Services</h3>
        ...
        <div className="rounded-2xl bg-background border border-border p-4 sm:p-6">
          <h2 className="text-base sm:text-lg font-semibold text-gray-950 dark:text-primary-foreground">Admin Priorities</h2>
