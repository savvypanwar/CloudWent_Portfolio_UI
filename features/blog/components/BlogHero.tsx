        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div data-reveal>
            <div className="inline-flex items-center gap-2 bg-card dark:bg-slate-800 border border-border dark:border-slate-700 rounded-full px-4 py-1.5 text-xs font-semibold text-muted-foreground dark:text-slate-400 shadow-card">
              <Sparkles className="w-3.5 h-3.5 text-primary dark:text-blue-400" /> Our Blog
            </div>
            <h1 className="mt-6 text-4xl sm:text-5xl md:text-6xl font-extrabold leading-[1.05] text-dark dark:text-white">
              Insights & Stories<br />
              <span className="text-primary dark:text-blue-400">From Our Team</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link href="/blog" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold shadow-md transition dark:bg-blue-500 dark:hover:bg-blue-600">
                Browse All Posts <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href="/blog/categories" className="inline-flex items-center gap-2 bg-card dark:bg-slate-800 border border-border dark:border-slate-700 px-6 py-3 rounded-xl font-semibold hover:bg-surface dark:hover:bg-slate-700 transition">
            ...
            <div className="mt-8 flex items-center gap-5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-bold ml-1 text-dark dark:text-white">5.0</span>
          ...
          <div className="relative" data-reveal data-delay="2">
            <Image src={heroImg} alt="CloudWent Blog" width={1024} height={1024} className="w-full max-w-xl mx-auto animate-float-slow drop-shadow-2xl" />
            <div className="absolute inset-0 pointer-events-none">
              {categories.map((cat, i) => (
                <div
                  key={cat.label}
                  className={`absolute ${cat.position} bg-card dark:bg-slate-800 rounded-2xl shadow-card border border-border dark:border-slate-700 px-3 py-2 flex items-center gap-2 animate-float`}
                  style={{ animationDelay: `${i * 0.4}s` }}
                >
                  <div className={`w-9 h-9 rounded-xl grid place-items-center bg-gradient-to-br ${cat.color} text-white text-xs font-bold`}>
                    {cat.label[0]}
                  </div>
                  <span className="text-xs font-semibold pr-1 text-dark dark:text-white">{cat.label}</span>
                </div>
              ))}
            </div>
          </div>
