      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold text-foreground">
            Simple, Respectful<br />Of Your Time
          </h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 lg:gap-8 relative">
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div
                key={step.id}
                className="relative text-center group card-hover border border-transparent hover:border-primary/30 rounded-2xl p-4 transition-all duration-300"
              >
                <div className="mx-auto w-20 h-20 rounded-full grid place-items-center glass-effect border-border shadow-sm group-hover:shadow-md transition">
                  <IconComponent
                    className={`w-8 h-8 ${colorPalette[i % colorPalette.length]} group-hover:scale-110 transition-transform duration-300`}
                  />
                </div>
                <div className="text-xs font-bold text-muted-foreground mt-3">{step.step}</div>
                <h3 className="font-bold text-lg mt-1 text-foreground group-hover:text-primary transition-colors duration-300">
                  {step.title}
                </h3>
