...
export const ContactHero = () => {
  ...
  return (
    <section className="relative overflow-hidden dark:bg-slate-950">
      {/* ✅ Light Mode Gradient */}
      ...
      <div className="max-w-7xl mx-auto px-6 pt-10 pb-10 grid lg:grid-cols-2 gap-12 items-center">
        <div data-reveal>
          ...
          <h1 className="mt-6 text-4xl sm:text-5xl md:text-6xl font-extrabold leading-[1.05] text-dark dark:text-white">
            ...
            <span className="text-blue-600 dark:text-blue-400">Amazing</span> Together
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="#form" className="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-3.5 rounded-xl font-semibold shadow-md hover:bg-blue-700 transition">
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 pointer-events-none">
            {badges.map((b, i) => (
              <div key={b.title} className={`absolute ${b.pos} bg-card dark:bg-slate-800 rounded-2xl shadow-card border border-border dark:border-slate-700 px-3 py-2.5 flex items-center gap-2.5 animate-float`} style={{ animationDelay: `${i * 0.5}s` }}>
                <span className="w-9 h-9 rounded-xl grid place-items-center bg-primary/10 dark:bg-primary/20 text-primary">
                  <b.icon className="w-4.5 h-4.5" />
                </span>
                <div>
                  <div className="text-xs font-bold leading-tight">{b.title}</div>
                  <div className="text-[10px] text-muted-foreground dark:text-slate-500">{b.desc}</div>
                </div>
              </div>
            ))}
          </div>
