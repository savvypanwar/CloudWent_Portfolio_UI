...
export const Hero = () => {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 pb-24 grid lg:grid-cols-2 gap-12 items-center">
        <div data-reveal>
          ...
          <h1 className="mt-6 text-4xl sm:text-5xl md:text-6xl font-extrabold leading-[1.05] text-dark">
            ...
            <span className="text-blue-600">For Ambitious</span><br />
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="#contact" className="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-3.5 rounded-xl font-semibold shadow-md hover:bg-blue-700 transition">
  Start Your Project <ArrowRight className="w-4 h-4" />
</a>
            <a href="#work" className="inline-flex items-center gap-2 bg-card border border-border px-6 py-3.5 rounded-xl font-semibold hover:bg-surface transition">
        ...
        {/* Right Column - Hero Image + Floating Icons */}
        <div className="relative">
          <img src={heroImg.src} alt="CloudWent platform" width={1024} height={1024} className="w-full max-w-xl mx-auto animate-float-slow drop-shadow-2xl" />
          
          <div className="absolute inset-0 pointer-events-none">
            {stack.map((s, i) => (
              <div 
                key={s.name} 
                className={`absolute ${s.position} bg-card rounded-2xl shadow-card border border-border px-3 py-2 flex items-center gap-2 animate-float`} 
                style={{ animationDelay: `${i * 0.4}s` }}
              >
                <img src={s.icon.src} alt={s.alt} className="w-9 h-9 object-contain" />
                <span className="text-xs font-semibold pr-1">{s.name}</span>
              </div>
            ))}
          </div>
        </div>
