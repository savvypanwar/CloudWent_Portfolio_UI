...
export const TeamHero = () => {
  // Team members as floating bubbles (like tech icons in Hero)
  const teamBubbles = [
      ...
      <div className="max-w-7xl mx-auto px-6 pt-10 pb-10 grid lg:grid-cols-2 gap-12 items-center">
        <div data-reveal>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href="#opportunities"
              className="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-3.5 rounded-xl font-semibold shadow-md hover:bg-blue-700 transition dark:bg-blue-500 dark:hover:bg-blue-600"
            >
              Join Our Team <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href="#work"
              className="inline-flex items-center gap-2 bg-card border border-border px-6 py-3.5 rounded-xl font-semibold hover:bg-surface transition dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700"
            >
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 pointer-events-none">
            {teamBubbles.map((bubble, i) => (
              <div
                key={bubble.initials}
                className={`absolute ${bubble.position} bg-card dark:bg-slate-800 rounded-2xl shadow-card border border-border dark:border-slate-700 px-3 py-2 flex items-center gap-2 animate-float`}
              ...
              >
                ...
                <span className="text-xs font-semibold pr-1 text-slate-800 dark:text-white">
                  {bubble.initials}
                </span>
              </div>
            ))}
